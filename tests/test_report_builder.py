...
def _make_section(summary, gaps=None):
    return ReportSection(
        ...
        key_points=[
            KeyPoint(
                claim="It sells widgets to UK retailers.",
                source_ids=[1],
...
def _make_research_result():
    ...
    registry.add(
        "companies_house",
        "Acme Ltd - Filing history",
        "https://public.example.test/company/123/filing-history",
    )
    report = StructuredReport(
        business_model=_make_section(
            "Acme Ltd sells widgets.",
            gaps=["No investor filings found."],
        ),
        competitive_landscape=_make_section("Moderately competitive market."),
        quality_signals=_make_section(
            "Limited review data.",
            gaps=["No Trustpilot presence found."],
        ),
    )
    return ResearchResult(report=report, sources=registry, tool_call_count=3)
...
def test_build_report_markdown_includes_sections_and_references():
    ...
    md = build_report_markdown(profile, result)
    ...
    assert md.count("No investor filings found.") == 1
